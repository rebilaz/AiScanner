...
async def run_coingecko_worker() -> None:
    ...
    try:
        ...
        async with aiohttp.ClientSession(connector=connector) as session:
            ...
            # ... la suite de la logique du worker pour traiter les `token_ids_to_fetch`
            
    except Exception as e:
        logging.critical(f"Une erreur fatale a arrêté le worker : {e}", exc_info=True)

if __name__ == "__main__":
    # Pour exécuter le worker en standalone
    # Note: L'utilisation de asyncio.run() est standard en Python 3.7+
    try:
