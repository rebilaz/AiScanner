"""On-chain analysis worker refactored for main integration."""
...
async def run_onchain_worker() -> bool:
    ...
    if not project_id or not dataset:
        print("Missing GCP configuration for on-chain worker")
        return False
    ...
    for _, row in batch.iterrows():
        project_id, name, chain, address, market_cap = row[
            "id_projet"
        ], row["nom"], row["chaine_contrat"], row["adresse_contrat"], row["market_cap"]
        ...
        if response.status_code != 200 or response.json().get("status") == "0":
            print("    -> Pas de données de transaction trouvées.")
            continue
...
def calculate_advanced_metrics(transactions: list[dict], project_id: str, market_cap: float, whale_usd: int) -> Dict[str, Any]:
