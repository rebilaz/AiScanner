...
DEST_LOCATION = os.getenv("BQ_DATASET_LOCATION", "US")
PUBLIC_LOCATION = os.getenv("ETH_PUBLIC_DATA_LOCATION", "US")
...
def sync_table(client: bigquery.Client, name: str, total_bytes: int) -> int:
    ...
    df, bytes_proc = extract_new_data(
        client, source, last_block, MAX_BLOCK_RANGE, location=PUBLIC_LOCATION
    )
    logging.info("%s query processed %.2f MB and returned %d rows", name, bytes_proc / 1_000_000, len(df))

    ingest_data(client, df, DEST_DATASET, dest, location=DEST_LOCATION)
...
async def run_eth_sync_worker() -> None:
    ...
    bq_client.ensure_dataset_exists(DEST_DATASET, DEST_LOCATION)
