...
with DAG(
    ...
    run_worker_2_1 = BashOperator(
        task_id="run_worker_2_1",
        bash_command="cd {{ dag.folder }}/.. && python -m workers.worker_2_1",
    )

    task_2_2 = BashOperator(
        task_id="run_worker_2_2",
        bash_command="cd {{ dag.folder }}/.. && python -m workers.worker_2_2",
    )

    task_2_3 = BashOperator(
        task_id="run_worker_2_3",
        bash_command="cd {{ dag.folder }}/.. && python -m workers.worker_2_3",
    )

    task_5_1 = BashOperator(
        task_id="run_worker_5_1",
        bash_command="cd {{ dag.folder }}/.. && python -m workers.worker_5_1",
    )

    task_5_2 = BashOperator(
        task_id="run_worker_5_2",
        bash_command="cd {{ dag.folder }}/.. && python -m workers.worker_5_2",
    )

    task_7_3 = BashOperator(
        task_id="run_worker_7_3",
        bash_command="cd {{ dag.folder }}/.. && python -m workers.worker_7_3",
    )

    task_4_1 = BashOperator(
        task_id="run_worker_4_1",
        bash_command="cd {{ dag.folder }}/.. && python -m workers.worker_4_1",
    )

    task_4_2 = BashOperator(
        task_id="run_worker_4_2",
        bash_command="cd {{ dag.folder }}/.. && python -m workers.worker_4_2",
    )

    task_4_3 = BashOperator(
        task_id="run_worker_4_3",
        bash_command="cd {{ dag.folder }}/.. && python -m workers.worker_4_3",
    )

    task_3_1 = BashOperator(
        task_id="run_worker_3_1",
        bash_command="cd {{ dag.folder }}/.. && python -m workers.worker_3_1",
    )

    task_6_3 = BashOperator(
        task_id="run_worker_6_3",
        bash_command="cd {{ dag.folder }}/.. && python -m workers.worker_6_3",
    )

    task_7_2 = BashOperator(
        task_id="run_worker_7_2",
        bash_command="cd {{ dag.folder }}/.. && python -m workers.worker_7_2",
    )

    task_3_2 = BashOperator(
        task_id="run_worker_3_2",
        bash_command="cd {{ dag.folder }}/.. && python -m workers.worker_3_2",
    )

    task_7_1 = BashOperator(
        task_id="run_worker_7_1",
        bash_command="cd {{ dag.folder }}/.. && python -m workers.worker_7_1",
    )

    task_7_4 = BashOperator(
        task_id="run_worker_7_4",
        bash_command="cd {{ dag.folder }}/.. && python -m workers.worker_7_4",
    )

    # Task dependencies defining the workflow order. Adjust these as the
    # architecture evolves.
    run_worker_2_1 >> task_2_2 >> task_2_3
    task_5_1 >> task_5_2
    task_2_2 >> task_7_3
